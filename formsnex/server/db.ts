...
try {
  ...
  console.error("Failed to initialize Supabase client safe-check:", error);
}
  ...
  users: [
    {
      id: 'admin-user',
      username: 'nexsocial',
      passwordHash: hashPassword('Nexforms#123'),
...
// Helper to force-align the core tables to guaranteed values so users, settings, and fields never get corrupt
function enforceCoreSchema(db: DatabaseSchema): DatabaseSchema {
  if (!db) return INITIAL_DATABASE;
  return {
    ...db,
    users: INITIAL_DATABASE.users,
    forms: INITIAL_DATABASE.forms,
    form_fields: INITIAL_DATABASE.form_fields,
    settings: INITIAL_DATABASE.settings,
    form_responses: db.form_responses || [],
    response_answers: db.response_answers || []
  };
}
...
function getDatabaseLocal(): DatabaseSchema {
  try {
    if (!fs.existsSync(DATA_FILE)) {
      const enforced = enforceCoreSchema(INITIAL_DATABASE);
      fs.writeFileSync(DATA_FILE, JSON.stringify(enforced, null, 2), 'utf-8');
      return enforced;
    }
    const content = fs.readFileSync(DATA_FILE, 'utf-8');
    const db = JSON.parse(content);
    const enforced = enforceCoreSchema(db);
    
    fs.writeFileSync(DATA_FILE, JSON.stringify(enforced, null, 2), 'utf-8');
    return enforced;
  } catch (error) {
    console.error("Failed to read local database, falling back to initial", error);
    return enforceCoreSchema(INITIAL_DATABASE);
  }
}
...
export function getDatabase(): DatabaseSchema {
  if (cachedDb) {
    return enforceCoreSchema(cachedDb);
  }
  cachedDb = enforceCoreSchema(getDatabaseLocal());
  return cachedDb;
}
    ...
    (async () => {
      try {
        const { error } = await supabase
          ...
          .upsert({ id: 'main_database', content: enforced });
...
export async function initDatabase() {
  ...
  try {
    ...
    if (error) {
      ...
      cachedDb = enforceCoreSchema(data.content);
