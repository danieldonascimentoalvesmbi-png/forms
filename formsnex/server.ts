...
async function startServer() {
  // Sync latest state from cloud Supabase in background so it never blocks express listener
  initDatabase().catch(err => {
    console.error("Async initDatabase failed on startup:", err);
  });
...
if (process.env.VERCEL) {
  // Safe async initialization on Vercel
